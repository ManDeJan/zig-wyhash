...
fn read_rest(data: []const u8) u64 {
...
pub fn hash_(key: []const u8, initial_seed: u64) u64 {
    const len = key.len;
    ...
    var i: usize = 0;
    while (i + 32 <= key.len) : (i += 32) {
        seed = mum(seed ^ primes[0], mum(read_bytes(8, key[i..]) ^ primes[1], read_bytes(8, key[i + 8 ..]) ^ primes[2]) ^
            mum(read_bytes(8, key[i + 16 ..]) ^ primes[3], read_bytes(8, key[i + 24 ..]) ^ primes[4]));
    }
    seed ^= primes[0];
...
pub fn hash__(key: []const u8, initial_seed: u64) u64 {
    const len = key.len;
    ...
    const rem_bytes = @truncate(u2, len & 31 / 8);
    const rem_bits  = @truncate(u3, len & 31 % 8);
    const rem_key   = key[rem_bytes * 8..];
    
    const rest = switch (rem_bits) {
        0 => read_8bytes_swapped(key[i + 8 ..]),
        1, 2, 4 => read_bytes(rem_bits, rem_key),
